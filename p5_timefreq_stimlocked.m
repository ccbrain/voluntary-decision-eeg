...
ConditionName = {'Equal100', 'Equal80', 'Equal20', ...
    'Control100', 'Control80', 'Control20'};
...
for sub_idx = 1:length(subjects)
    ...
    for j= 1:length(ConditionName)
        ...
        for nr_electrode = 1:length(EEG.chanlocs)
            [ersp{nr_electrode} itc powbase tm fq] = pop_newtimef( EEG, 1, nr_electrode, [-500  800], [3  0.5] , ...
                'topovec', 1, 'elocs', EEG.chanlocs, 'chaninfo', EEG.chaninfo, ...
                'caption', [field ' electr:' EEG.chanlocs(nr_electrode).labels], ...
                'baseline', [-400 -100], 'freqs', [0 55], ...
                'plotitc' , 'off', 'plotphase', 'off', 'plotersp', 'off', 'padratio', 1, 'winsize', 150);
            fig = plot_spectrogram(tm, fq, ersp{nr_electrode}, [field ' electr:' EEG.chanlocs(nr_electrode).labels], 1);
